...
class Backtester:
    ...
    def __init__(
        ...
        start_deposit,
        leverage,
        open_position_fee_percent,
        close_position_fee_percent,
        use_long_positions,
        use_short_positions,
        take_profit_percents_list,
        stop_loss_percents_list,
        start_year,
        start_month,
        start_day,
        start_hour,
        ...
        minimum_number_of_candles_to_start_trading,
        important_recent_candles_timeframe,
        important_recent_candles_count,
        coin_maximum_price,
        open_position_timeframe,
        test_set_size_ratio,
        plot_file_path,
        positions_csv_report_file_path,
        deposit_changes_csv_report_file_path,
        candles_plot_file_path,
    ):
        ...
        self.minimum_number_of_candles_to_start_trading = (
            minimum_number_of_candles_to_start_trading
        )
    ...
    def _download_or_load_candles(self, timeframe):
        ...
        if os.path.isfile(self.candles_file_path[timeframe]):
            self.candles_list[timeframe] = pickle.load(
                open(self.candles_file_path[timeframe], "rb")
            )
        else:
            while True:
                try:
                    self.candles_list[timeframe] = binance_api.get_candles(
                        timeframe,
                        self.start_date_timestamp,
                        self.end_date_timestamp,
                        self.coins_symbol,
                    )
                    pickle.dump(
                        self.candles_list[timeframe],
                        open(self.candles_file_path[timeframe], "wb"),
                    )
                    break
                except:
    ...
    def _update_candles_statistics(self):
        self.candles_open_to_close_list.append(
            self.current_candle.close - self.current_candle.open
        )
        self.candles_open_to_close_abs_list.append(
            abs(self.current_candle.close - self.current_candle.open)
        )
        self.candles_open_to_close_random_signed_list.append(
            random.randint(-1, 1)
            * abs(self.current_candle.close - self.current_candle.open)
        )
        self.candles_open_to_low_list.append(
            self.current_candle.low - self.current_candle.open
        )
        self.candles_open_to_high_list.append(
            self.current_candle.high - self.current_candle.open
        )
        self.candles_low_to_high_list.append(
            self.current_candle.high - self.current_candle.low
        )
        self.candles_low_and_high_to_open_list.append(
            self.current_candle.low - self.current_candle.open
        )
        self.candles_low_and_high_to_open_list.append(
            self.current_candle.high - self.current_candle.open
        )
        self.candles_low_and_high_to_open_abs_list.append(
            abs(self.current_candle.low - self.current_candle.open)
        )
        self.candles_low_and_high_to_open_abs_list.append(
            abs(self.current_candle.high - self.current_candle.open)
        )

        self.candles_open_to_close_percent_list.append(
            100
            * (self.current_candle.close - self.current_candle.open)
            / self.current_candle.open
        )
        self.candles_open_to_close_abs_percent_list.append(
            100
            * abs(self.current_candle.close - self.current_candle.open)
            / self.current_candle.open
        )
        self.candles_open_to_close_random_signed_percent_list.append(
            random.randint(-1, 1)
            * 100
            * abs(self.current_candle.close - self.current_candle.open)
            / self.current_candle.open
        )
        self.candles_open_to_low_percent_list.append(
            100
            * (self.current_candle.low - self.current_candle.open)
            / self.current_candle.open
        )
        self.candles_open_to_high_percent_list.append(
            100
            * (self.current_candle.high - self.current_candle.open)
            / self.current_candle.open
        )
        self.candles_low_to_high_percent_list.append(
            100
            * (self.current_candle.high - self.current_candle.low)
            / self.current_candle.low
        )
        self.candles_low_and_high_to_open_percent_list.append(
            100
            * (self.current_candle.low - self.current_candle.open)
            / self.current_candle.open
        )
        ...
        self.candles_low_and_high_to_open_abs_percent_list.append(
            100
            * abs(self.current_candle.low - self.current_candle.open)
            / self.current_candle.open
        )
    ...
    def _init_index_dicts(self):
        ...
        for tf in self.all_timeframes_list:
            ...
            for i in range(len(self.candles_list[tf])):
                self.candles_open_time_to_index_dict[tf][
                    self.candles_list[tf][i].open_time
                ] = i

    def _is_it_time_to_update_indicators(self):
        if self.timeframe == "m1" and self.indicators_timeframe == "m15":
            return self.current_candle.open_time == utils.round_down_m1_to_m15_time(
                self.current_candle.open_time
            )
        else:
            return True
    ...
    def _update_important_recent_candles(self, important_recent_candles_timeframe):
        if important_recent_candles_timeframe == "m15":
            try:
                self.important_recent_candles_list = self.candles_list["m15"][
                    max(
                        self.candles_open_time_to_index_dict["m15"][
                            utils.round_down_m1_to_m15_time(
                                self.current_candle.open_time
                            )
                        ]
                        - self.important_recent_candles_count,
                        0,
                    ) : self.candles_open_time_to_index_dict["m15"][
                        utils.round_down_m1_to_m15_time(self.current_candle.open_time)
                    ]
                ]
            except:
        ...
        if important_recent_candles_timeframe == "h1":
            try:
                self.important_recent_candles_list = self.candles_list["h1"][
                    max(
                        self.candles_open_time_to_index_dict["h1"][
                            utils.round_down_m1_to_h1_time(
                                self.current_candle.open_time
                            )
                        ]
                        - self.important_recent_candles_count,
                        ...
                        utils.round_down_m1_to_h1_time(self.current_candle.open_time)
                    ]
                ]
            except:
                print("ERROR in update_important_recent_candles")
        ...
        if important_recent_candles_timeframe == "h2":
            try:
                self.important_recent_candles_list = self.candles_list["h2"][
                    max(
                        self.candles_open_time_to_index_dict["h2"][
                            utils.round_down_m1_to_h2_time(
                                self.current_candle.open_time
                            )
                        ]
                        - self.important_recent_candles_count,
                        ...
                        utils.round_down_m1_to_h2_time(self.current_candle.open_time)
                    ]
                ]
            except:
                print("ERROR in update_important_recent_candles")
        ...
        if important_recent_candles_timeframe == "h4":
            try:
                self.important_recent_candles_list = self.candles_list["h4"][
                    max(
                        self.candles_open_time_to_index_dict["h4"][
                            utils.round_down_m1_to_h4_time(
                                self.current_candle.open_time
                            )
                        ]
                        - self.important_recent_candles_count,
                        ...
                        utils.round_down_m1_to_h4_time(self.current_candle.open_time)
                    ]
                ]
            except:
                print("ERROR in update_important_recent_candles")
        ...
        if important_recent_candles_timeframe == "d1":
            try:
                self.important_recent_candles_list = self.candles_list["d1"][
                    max(
                        self.candles_open_time_to_index_dict["d1"][
                            utils.round_down_m1_to_d1_time(
                                self.current_candle.open_time
                            )
                        ]
                        - self.important_recent_candles_count,
                        ...
                        utils.round_down_m1_to_d1_time(self.current_candle.open_time)
                    ]
                ]
            except:
                print("ERROR in update_important_recent_candles")
        ...
        self.recent_open_prices_list = [
            candle.open / self.coin_maximum_price
            for candle in self.important_recent_candles_list
        ]
        self.recent_high_prices_list = [
            candle.high / self.coin_maximum_price
            for candle in self.important_recent_candles_list
        ]
        self.recent_low_prices_list = [
            candle.low / self.coin_maximum_price
            for candle in self.important_recent_candles_list
        ]
        self.recent_close_prices_list = [
            candle.close / self.coin_maximum_price
            for candle in self.important_recent_candles_list
        ]
    ...
    def _get_current_candle_in_another_timeframe(self, timeframe, index_offset):
        try:
            if timeframe == "m15":
                return self.candles_list[timeframe][
                    self.candles_open_time_to_index_dict[timeframe][
                        utils.round_down_m1_to_m15_time(self.current_candle.open_time)
                    ]
                    + index_offset
                ]
    ...
    def _update_indicators(self):
        ...
        self.fast_ema = indicators.get_new_ema(
            self.last_fast_ema,
            self._get_current_candle_in_another_timeframe(
                timeframe=self.indicators_timeframe, index_offset=-1
            ).close,
            1 * config.MOVING_AVERAGE_SIZE,
        )
        self.slow_ema = indicators.get_new_ema(
            self.last_slow_ema,
            self._get_current_candle_in_another_timeframe(
                timeframe=self.indicators_timeframe, index_offset=-1
            ).close,
            2 * config.MOVING_AVERAGE_SIZE,
        )

        self.macd_fast_ema = indicators.get_new_ema(
            self.last_macd_fast_ema,
            self._get_current_candle_in_another_timeframe(
                timeframe=self.indicators_timeframe, index_offset=-1
            ).close,
            12,
        )
        self.macd_slow_ema = indicators.get_new_ema(
            self.last_macd_slow_ema,
            self._get_current_candle_in_another_timeframe(
                timeframe=self.indicators_timeframe, index_offset=-1
            ).close,
            26,
        ...
        self.signal_line = indicators.get_new_ema(
            self.last_signal_line, self.macd_line, 9
        )
    ...
    def _update_plot_deposit_lists(self):
        self.plot_deposits_list.append(
            self.total_first_coins + self.open_positions_value
        )
    ...
    def _is_it_time_to_open_long_position(self):
        ...
        is_too_early = (
            self.candles_index < self.minimum_number_of_candles_to_start_trading
        )
        is_positions_list_empty = (
            len(self.open_long_positions_list) == 0
            and len(self.open_short_positions_list) == 0
        )
        is_ontime = False
        ...
        if self.open_position_timeframe == "h1":
            is_ontime = self.current_candle.open_time == utils.round_down_m1_to_h1_time(
                self.current_candle.open_time
            )
        if self.open_position_timeframe == "h2":
            is_ontime = self.current_candle.open_time == utils.round_down_m1_to_h2_time(
                self.current_candle.open_time
            )
        if self.open_position_timeframe == "h4":
            is_ontime = self.current_candle.open_time == utils.round_down_m1_to_h4_time(
                self.current_candle.open_time
            )
        if self.open_position_timeframe == "d1":
            is_ontime = self.current_candle.open_time == utils.round_down_m1_to_d1_time(
                self.current_candle.open_time
            )
        return (
            is_positions_list_empty
            and is_ontime
            and not is_too_early
            and main_condition
        )
    ...
    def _update_plot_candles_list(self):
        ...
        if is_ontime:
            candle = self._get_current_candle_in_another_timeframe(
                timeframe=self.open_position_timeframe, index_offset=0
            )
    ...
    def _open_long_position(self):
        for i in range(self.maximum_open_positions_count):
            current_long_position = position.Position(
                position_type="long",
                entry_time=self.current_candle.open_time,
                exit_time="",
                leverage=config.LEVERAGE,
                first_coins_in_position=(
                    self.total_first_coins / self.maximum_open_positions_count
                )
                * (1 - self.open_position_fee_percent / 100),
                ...
                take_profit_price=self.current_candle.open
                * (1 + self.take_profit_percents_list[i] / 100),
                stop_loss_price=self.current_candle.open
                * (1 + self.stop_loss_percents_list[i] / 100),
    ...
    def _open_short_position(self):
        for i in range(self.maximum_open_positions_count):
            current_short_position = position.Position(
                position_type="short",
                entry_time=self.current_candle.open_time,
                exit_time="",
                leverage=config.LEVERAGE,
    ...
    def _update_open_long_positions_statistics(self):
        ...
        for i in range(open_long_positions_count):
            self.open_long_positions_list[i].max_profit_percent = max(
                self.open_long_positions_list[i].max_profit_percent,
                100
                * (
                    self.current_candle.close
                    - self.open_long_positions_list[i].entry_price
                )
                / self.open_long_positions_list[i].entry_price,
            )
            self.open_long_positions_list[i].min_profit_percent = min(
                self.open_long_positions_list[i].min_profit_percent,
                100
                * (
                    self.current_candle.close
                    - self.open_long_positions_list[i].entry_price
    ...
    def _update_open_short_positions_statistics(self):
        ...
        for i in range(open_short_positions_count):
            self.open_short_positions_list[i].max_profit_percent = max(
                self.open_short_positions_list[i].max_profit_percent,
                -100
                * (
                    self.current_candle.close
                    - self.open_short_positions_list[i].entry_price
                )
                / self.open_short_positions_list[i].entry_price,
            )
            self.open_short_positions_list[i].min_profit_percent = min(
                self.open_short_positions_list[i].min_profit_percent,
                -100
                * (
                    self.current_candle.close
                    - self.open_short_positions_list[i].entry_price
    ...
    def _check_conditions_to_close_long_position(self):
        ...
        for i in reversed(range(open_long_positions_count)):
            if (
                ...
                > self.open_long_positions_list[i].take_profit_price
                or self.current_candle.low
                < self.open_long_positions_list[i].stop_loss_price
                or main_condition
            ):
                self.last_position_status += self.CLOSE_LONG_POSITION
                recent_candles_list = self.open_long_positions_list[
                    i
                ].recent_candles_list
                recent_candles_close_list = [
                    candle.close for candle in recent_candles_list
                ]
                recent_candles_open_list = [
                    candle.open for candle in recent_candles_list
                ]
                recent_candles_high_list = [
                    candle.high for candle in recent_candles_list
                ]
                recent_candles_low_list = [candle.low for candle in recent_candles_list]
                position_result_and_candles = PositionResultAndCandles(
                    open_prices_list=recent_candles_open_list,
                ...
                self.open_long_positions_list[
                    i
                ].exit_time = self.current_candle.close_time
                if (
                    self.current_candle.low
                    < self.open_long_positions_list[i].stop_loss_price
                ):
                    self.open_long_positions_list[
                        i
                    ].exit_price = self.open_long_positions_list[i].stop_loss_price
                    ...
                    self.open_long_positions_list[
                        i
                    ].exit_price = self.open_long_positions_list[i].take_profit_price
                ...
                if len(recent_candles_list) == self.important_recent_candles_count:
                    self.position_result_and_candles_list.append(
                        position_result_and_candles
                    )
                self.open_long_positions_list[i].profit_percent = (
                    100
                    * (
                        self.open_long_positions_list[i].exit_price
                        - self.open_long_positions_list[i].entry_price
                    )
                    / self.open_long_positions_list[i].entry_price
                )
                current_position_value = (
                    1
                    + self.open_long_positions_list[i].profit_percent
                    * self.open_long_positions_list[i].leverage
                    / 100
                ) * self.open_long_positions_list[i].first_coins_in_position
                current_position_profit = (
                    self.open_long_positions_list[i].profit_percent
                    * self.open_long_positions_list[i].leverage
                    / 100
                ) * self.open_long_positions_list[i].first_coins_in_position
                ...
                self.total_first_coins += current_position_value * (
                    1 - self.close_position_fee_percent / 100
                )
    ...
    def _check_conditions_to_close_short_position(self):
        ...
        for i in reversed(range(open_short_positions_count)):
            if (
                self.current_candle.low
                < self.open_short_positions_list[i].take_profit_price
                or self.current_candle.high
                > self.open_short_positions_list[i].stop_loss_price
                or main_condition
            ):
                self.last_position_status += self.CLOSE_SHORT_POSITION
                recent_candles_list = self.open_short_positions_list[
                    i
                ].recent_candles_list
                recent_candles_close_list = [
                    candle.close for candle in recent_candles_list
                ...
                if (
                    ...
                    self.open_short_positions_list[
                        i
                    ].exit_price = self.open_short_positions_list[i].stop_loss_price
                    ...
                    self.open_short_positions_list[
                        i
                    ].exit_price = self.open_short_positions_list[i].take_profit_price
                ...
                self.open_short_positions_list[i].profit_percent = (
                    100
                    * (
                        self.open_short_positions_list[i].entry_price
                        - self.open_short_positions_list[i].exit_price
                    )
                    / self.open_short_positions_list[i].entry_price
                )
                current_position_value = (
                    1
                    + self.open_short_positions_list[i].profit_percent
                    * self.open_short_positions_list[i].leverage
                    / 100
                ) * self.open_short_positions_list[i].first_coins_in_position
                current_position_profit = (
                    self.open_short_positions_list[i].profit_percent
                    * self.open_short_positions_list[i].leverage
                    / 100
    ...
    def _update_open_long_positions_value(self):
        ...
        open_long_positions_count = len(self.open_long_positions_list)
        for i in range(open_long_positions_count):
            self.open_long_positions_value += (
                1
                + config.LEVERAGE
                * (
                    self.current_candle.close
        ...
        self.open_positions_value = (
            self.open_short_positions_value + self.open_long_positions_value
        )
    ...
    def _update_open_short_positions_value(self):
        ...
        open_short_positions_count = len(self.open_short_positions_list)
        for i in range(open_short_positions_count):
            self.open_short_positions_value += (
                1
                - config.LEVERAGE
                * (
                    self.current_candle.close
    ...
    def _print_main_backtest_results(self):
        ...
        if len(self.closed_positions_list) > 0:
            print(
                "win rate percent:",
                100 * self.total_wins / len(self.closed_positions_list),
            )
            print(
                "take profits win rate percent:",
                100 * self.total_take_profit_wins / len(self.closed_positions_list),
                "\n",
            )
        else:
    ...
    def _prepare_train_set_and_test_set(self):
        self.train_set_list = []
        self.test_set_list = []
        position_result_and_candles_list_len = len(
            self.position_result_and_candles_list
        )
        ...
        for i in range(position_result_and_candles_list_len):
            if (
                i
                >= (1 - self.test_set_size_ratio) * position_result_and_candles_list_len
            ):
    ...
    def _show_and_save_candles_plot_to_file(self):
        plot_maker.PlotMaker.add_candles_plot(
            candle.Candle.candles_list_to_pandas_dataframe(self.plot_candles_list),
            self.plot_candle_colors_list,
        )
        plot_maker.PlotMaker.save_candles_plot(
            candle.Candle.candles_list_to_pandas_dataframe(self.plot_candles_list),
            self.plot_candle_colors_list,
            self.candles_plot_file_path,
        )
    ...
    def _print_candles_statistical_parameters(self):
        report_maker.print_statistical_parameters(
            "candles_open_to_close_list",
            self.candles_open_to_close_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_open_to_close_abs_list",
            self.candles_open_to_close_abs_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_open_to_close_random_signed_list",
            self.candles_open_to_close_random_signed_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_open_to_low_list",
            self.candles_open_to_low_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_open_to_high_list",
            self.candles_open_to_high_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_low_to_high_list",
            self.candles_low_to_high_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_low_and_high_to_open_list",
            self.candles_low_and_high_to_open_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_low_and_high_to_open_abs_list",
            self.candles_low_and_high_to_open_abs_list,
            self.report_percentiles_count,
        )

        report_maker.print_statistical_parameters(
            "candles_open_to_close_percent_list",
            self.candles_open_to_close_percent_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_open_to_close_abs_percent_list",
            self.candles_open_to_close_abs_percent_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_open_to_close_random_signed_percent_list",
            self.candles_open_to_close_random_signed_percent_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_open_to_low_percent_list",
            self.candles_open_to_low_percent_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_open_to_high_percent_list",
            self.candles_open_to_high_percent_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_low_to_high_percent_list",
            self.candles_low_to_high_percent_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_low_and_high_to_open_percent_list",
            self.candles_low_and_high_to_open_percent_list,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "candles_low_and_high_to_open_abs_percent_list",
            self.candles_low_and_high_to_open_abs_percent_list,
            self.report_percentiles_count,
        )
    ...
    def _prepare_closed_positions_statistics(self):
        ...
        for closed_position in self.closed_positions_list:
            self.closed_positions_max_profit_percents_list.append(
                closed_position.max_profit_percent
            )
            self.closed_positions_min_profit_percents_list.append(
                closed_position.min_profit_percent
            )
            self.closed_positions_profit_percents_list.append(
                closed_position.profit_percent
            )
    ...
    def _prepare_deposit_changes_statistics(self):
        ...
        for i in range(len(self.plot_deposits_list)):
            if i >= utils.convert_candles_count(self.timeframe, "d1"):
                self.deposit_daily_changes_percent.append(
                    100
                    * (
                        self.plot_deposits_list[i]
                        - self.plot_deposits_list[
                            i - utils.convert_candles_count(self.timeframe, "d1")
                        ]
                    )
                    / self.plot_deposits_list[
                        i - utils.convert_candles_count(self.timeframe, "d1")
            ...
            if i >= utils.convert_candles_count(self.timeframe, "w1"):
                self.deposit_weekly_changes_percent.append(
                    100
                    * (
                        self.plot_deposits_list[i]
                        - self.plot_deposits_list[
                            i - utils.convert_candles_count(self.timeframe, "w1")
                        ]
                    )
                    / self.plot_deposits_list[
                        i - utils.convert_candles_count(self.timeframe, "w1")
            ...
            if i >= utils.convert_candles_count(self.timeframe, "w2"):
                self.deposit_biweekly_changes_percent.append(
                    100
                    * (
                        self.plot_deposits_list[i]
                        - self.plot_deposits_list[
                            i - utils.convert_candles_count(self.timeframe, "w2")
                        ]
                    )
                    / self.plot_deposits_list[
                        i - utils.convert_candles_count(self.timeframe, "w2")
            ...
            if i >= utils.convert_candles_count(self.timeframe, "M1"):
                self.deposit_monthly_changes_percent.append(
                    100
                    * (
                        self.plot_deposits_list[i]
                        - self.plot_deposits_list[
                            i - utils.convert_candles_count(self.timeframe, "M1")
                        ]
                    )
                    / self.plot_deposits_list[
                        i - utils.convert_candles_count(self.timeframe, "M1")
            ...
            if i >= utils.convert_candles_count(self.timeframe, "M2"):
                self.deposit_bimonthly_changes_percent.append(
                    100
                    * (
                        self.plot_deposits_list[i]
                        - self.plot_deposits_list[
                            i - utils.convert_candles_count(self.timeframe, "M2")
                        ]
                    )
                    / self.plot_deposits_list[
                        i - utils.convert_candles_count(self.timeframe, "M2")
            ...
            if i >= utils.convert_candles_count(self.timeframe, "M3"):
                self.deposit_trimonthly_changes_percent.append(
                    100
                    * (
                        self.plot_deposits_list[i]
                        - self.plot_deposits_list[
                            i - utils.convert_candles_count(self.timeframe, "M3")
                        ]
                    )
                    / self.plot_deposits_list[
                        i - utils.convert_candles_count(self.timeframe, "M3")
            ...
            if i >= utils.convert_candles_count(self.timeframe, "y1"):
                self.deposit_yearly_changes_percent.append(
                    100
                    * (
                        self.plot_deposits_list[i]
                        - self.plot_deposits_list[
                            i - utils.convert_candles_count(self.timeframe, "y1")
                        ]
                    )
                    / self.plot_deposits_list[
                        i - utils.convert_candles_count(self.timeframe, "y1")
    ...
    def _print_deposit_changes_statistics(self):
        report_maker.print_statistical_parameters(
            "deposit_daily_changes_percent",
            self.deposit_daily_changes_percent,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "deposit_weekly_changes_percent",
            self.deposit_weekly_changes_percent,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "deposit_biweekly_changes_percent",
            self.deposit_biweekly_changes_percent,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "deposit_monthly_changes_percent",
            self.deposit_monthly_changes_percent,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "deposit_bimonthly_changes_percent",
            self.deposit_bimonthly_changes_percent,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "deposit_trimonthly_changes_percent",
            self.deposit_trimonthly_changes_percent,
            self.report_percentiles_count,
        )
        report_maker.print_statistical_parameters(
            "deposit_yearly_changes_percent",
            self.deposit_yearly_changes_percent,
            self.report_percentiles_count,
        )
    ...
    def _save_closed_positions_to_csv(self):
        ...
        report_maker.generate_positions_csv_report(
            self.positions_csv_report_file_path,
            self.POSITIONS_CSV_REPORT_COLUMN_NAMES,
            self.closed_positions_list,
        )
    ...
    def _save_deposit_changes_to_csv(self):
        ...
        report_maker.generate_deposit_changes_csv_report(
            self.deposit_changes_csv_report_file_path,
            self.DEPOSIT_CHANGES_CSV_REPORT_COLUMN_NAMES,
            self.start_deposit,
            self.final_deposit,
            self.deposit_monthly_changes_percent,
            self.deposit_trimonthly_changes_percent,
        ...
        print(
            "Writing to " + self.deposit_changes_csv_report_file_path + " finished.\n"
        )
    ...
    def _iterate_candles(self):
        ...
        for candles_index in range(1, len(self.candles_list[self.timeframe]) - 1):
            ...
            last_progress_percent = int(
                100 * (candles_index - 1) / len(self.candles_list[self.timeframe])
            )
            progress_percent = int(
                100 * (candles_index) / len(self.candles_list[self.timeframe])
            )
            ...
            self._update_important_recent_candles(
                self.important_recent_candles_timeframe
            )
